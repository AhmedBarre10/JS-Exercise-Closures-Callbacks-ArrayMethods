...
function processSum(numberList, callback) {
  let reducing = callback(numberList.reduce(function(item1, item2){
    return item1 + item2;
  },0))
  return reducing
 }
...
function processContains(item, list, callback) {
  // check inside list if item is included if return callback (true) el
 ...
 * 
 * @param runners array of runners like the one inside thhe /data/runners.js file.
...
function getFullNames(runners) {
    ...
    runners.forEach(function(item){
      return runshirt.push(`${item.last_name}, ${item.first_name}`)
...
let cities = runners.filter(val => val.shirt_size === tShirtSize);
  return cities;
}
...
function tallyUpDonations(runners) {
  /* CODE HERE */
  // for each object inside array, access the donations
  let dono = []
   dono = runners.map(function(runner) {
     return runner.donation;
   });
  return dono.reduce((function(first, second){return first + second;}),0);
  
   
   
    //  let redu = runners.reduce () => {
       
    //   return runners.start;
    // },0);
    // return redu;
